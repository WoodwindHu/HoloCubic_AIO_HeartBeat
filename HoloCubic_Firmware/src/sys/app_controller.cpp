...
int AppController::app_install(APP_OBJ *app) // 将APP安装到app_controller中
{
    ...
    appList[app_num] = app;
    Serial.printf("%s installed\n", app->app_name);
    if (!strcmp(app->app_name,"Heartbeat")) {
        heartbeat_app_index = app_num;
        Serial.printf("heartbeat_app_index is %d\n", heartbeat_app_index);
    }
    ++app_num;
    return 0; // 安装成功
}
...
int AppController::main_process(ImuAction *act_info)
{
    ...
    // 重新 连接mqtt
    if (true == m_wifi_status && doDelayMillisTime(1000, &m_premqttReqMillis, false))
    {
...
bool AppController::wifi_event(APP_MESSAGE_TYPE type)
{
    ...
    case APP_MESSAGE_MQTT_DATA:
    {
        ...
        if (app_exit_flag == 1 && cur_app_index != heartbeat_app_index) // 在其他app中
        {
            app_exit_flag = 0;
            Serial.printf("heartbeat_app_index is %d\n", heartbeat_app_index);
        ...
        if (app_exit_flag == 0)
        {
            ...
            cur_app_index = heartbeat_app_index; // TODO
            (*(appList[cur_app_index]->app_init))(); // 执行APP初始化
        }
        // send_to("Heartbeat", "Heartbeat",  APP_MESSAGE_MQTT_DATA, 0, NULL); // 向Heartbeat发送收到MQTT消息
    }
    break;
    default:
        break;
